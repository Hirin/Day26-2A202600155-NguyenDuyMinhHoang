...
def check_routing(state: State) -> list[Send]:
    ...
    if any(kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu", "rò rỉ", "bảo mật", "bảo vệ dữ liệu"]):
...
def privacy_agent(state: State) -> dict:
    llm = get_llm()
    prompt = f"""Bạn là chuyên gia bảo vệ dữ liệu cá nhân. Phân tích khía cạnh quyền riêng tư:

Câu hỏi: {state['question']}
Phân tích pháp lý: {state.get('law_analysis', 'N/A')}

Tập trung: GDPR, CCPA, data protection, privacy rights, data breach notification, consent, right to be forgotten."""
